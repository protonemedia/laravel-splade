...
class BladeDirectives
{
    ...
    public function splade($expression = '')
    {
        ...
        $template[] = '<div id="layout" /><div id="' . $id . '" data-components="{{ json_encode($components) }}" data-html="{{ json_encode($html) }}" data-dynamics="{{ json_encode($dynamics) }}" data-splade="{{ json_encode($splade) }}">';
        $template[] = '{!! $ssrBody !!}';
        $template[] = '</div>';
