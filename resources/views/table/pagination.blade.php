    ...
    <div class="hidden md:flex-1 md:flex md:items-center md:justify-between">
        ...
        <div>
            <span class="relative z-0 inline-flex shadow-sm rounded-md">
                        ...
                        @foreach ($element as $page => $url)
                            @if ($page == $paginator->currentPage())
                                <span aria-current="page">
                                    <span class="relative inline-flex items-center px-4 py-2 -ml-px text-xs sm:text-sm font-medium dark:bg-gray-700 dark:text-white dark:border-gray-600 bg-indigo-50 border-indigo-500 text-indigo-600 z-10 border cursor-default leading-5">{{ $page }}</span>
                                </span>
                            @else
                ...
                {{-- Next Page Link --}}
                @if ($paginator->hasMorePages())
                    <Link keep-modal dusk="pagination-next" href="{{ $paginator->nextPageUrl() }}" rel="next" class="relative inline-flex items-center px-2 py-2 -ml-px text-xs sm:text-sm font-medium text-gray-500 bg-white dark:bg-gray-700 dark:text-white dark:border-gray-600 border border-gray-300 ltr:rounded-l-md rtl:rounded-r-md leading-5 hover:text-gray-400 focus:z-10 focus:outline-none focus:ring ring-gray-300 focus:border-blue-300 active:bg-gray-100 active:text-gray-500 transition ease-in-out duration-150" aria-label="{{ __('pagination.next') }}">
                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
