...
export default {
    ...
    install: (app, options) => {
        ...
        options.surpress_compile_errors = has(options, "surpress_compile_errors") ? options.surpress_compile_errors : false;
        ...
        if(options.surpress_compile_errors) {
            app.config.compilerOptions.onError = (error) => {
                import("./CompilerErrorMessages.js").then((CompilerErrorMessages) => {
                    console.error({
                        message:  CompilerErrorMessages.default[error.code] || "Unknown compiler error",
