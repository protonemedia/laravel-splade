...
function init(initialHtml, initialDynamics, initialSpladeData) {
    ...
    onHtml(initialHtml, 0, initialDynamics);
...
function handlePopstateEvent($event) {
    ...
    onHtml(currentPage.value.html, currentPage.value.rememberedState.scrollY, currentPage.value.dynamics);
}
...
function newPageFromResponse(response, replace) {
    ...
    const hasDynamics = Object.keys(dynamics).length > 0;
    ...
    onHtml(html, 0, dynamics); // supply html to app
...
function onHtml(html, scrollY, dynamics) {
    onHtmlFunction.value(html, scrollY, dynamics);
}
