...
function newPageFromResponse(response, replace, hash) {
    ...
    if (!isSsr && response.data.splade.preserveScroll) {
    ...
    onHtml(
        ...
        scrollY,
        !replace && !response.data.splade.preventViewTransition
    );
 ...
 */
function getSpladeDataFromLocalStorage() {
...
function storeInLocalStorage(key, data) {
    let allData = getSpladeDataFromLocalStorage();
...
function restore(key, useLocalStorage) {
    if (useLocalStorage) {
        const spladeData = getSpladeDataFromLocalStorage();
...
function request(url, method, data, headers, replace, blob) {
    ...
    promise
        ...
        .catch(async (error) => {
            ...
            if (response.status == 409 && response.headers["x-splade-redirect-away"]) {
                return window.location = response.headers["x-splade-redirect-away"];
            }
            ...
            if (response.data instanceof Blob) {
                ...
                if (response.data.type === "application/json" && typeof responseText === "string") {
                    spladeData = JSON.parse(responseText) || {};
                } else {
                ...
                spladeData = response.data.splade;
            }
